...
term <- "refugee" # Need to use + to string together separate words however it is concatinating
begin_date <- "20120101"
end_date <- "20120201"#change to 2 months to get data faster
#glocations <- "U.S"
```

#Searching for the terms that we query above and essentially creating the query object in order to use it for the API

# to add geolocation use "&glocations=",glocations,

#"&fq" = "immigration" ,

#"&fq=", "immigration OR refugee" , doesn't even like this?
```{r}
...
for(i in 0:maxPages){
  ...
  pages[[i+1]] <- nytSearch 
  Sys.sleep(5) #change to 5 to test on smaller datasets, 30 for actual execution
}
```
```{r}
#as.data.frame(nyt)
...
not_news_variable <- c("Op-Ed","Editorial","Interactive Feature", "Review", "Letter", "Correction")
